...
#. Go to *Sales > Products > Products*, create or select a product and check
   *Is Pack?*
#. Set "Pack type" to *Non Detailed - Assisted* in the *Pack* page.
#. Add the products to be included in it.
#. Go to *Sales > Orders > Quotations* and create a Quotation.
#. Add a product that has *Pack type* set to *Non Detailed - Assisted*.
#. Click the "Detail" button that appears on the sale order line.
#. A wizard will open showing all pack components with their quantities, prices,
   and discounts where you can adjust them as needed.
#. Click "Transform to Detail Lines" to convert the pack into:

   * A section line with the pack name (with hidden composition by default)
   * Individual sale order lines for each component with the configured quantities
     and discounts
